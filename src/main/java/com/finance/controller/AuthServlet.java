...
public class AuthServlet extends HttpServlet {
        ...
        if (user != null) {
            ...
            switch (user.getRole()) {
                ...
                case "USER":
                    ...
                    break;
                default:
                    resp.sendRedirect(req.getContextPath() + "/login.jsp?error=Unknown role");
            }
        } else {
    ...
    private void register(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
        ...
        // Password Validation
        // Regex: At least 8 chars, 1 number, 1 uppercase, 1 special char (@#$%!)
        String passwordPattern = "(?=.*\\d)(?=.*[A-Z])(?=.*[@#$%!]).{8,}";
        if (!password.matches(passwordPattern)) {
            req.setAttribute("error", "Password must be at least 8 characters and include 1 number, 1 uppercase letter, and 1 special character (@#$%!)");
        ...
        if (userDAO.createUser(user)) {
            resp.sendRedirect(req.getContextPath() + "/login.jsp?message=Registration successful, please login");
        } else {
